...
function Editor({
  ...
  const changeActiveOption = (currentState) => {
    ...
    if (currentStyle.has(inlineOptions.bold)) tempActiveOptions.bold = true;
    else tempActiveOptions.bold = false;

    if (currentStyle.has(inlineOptions.italic)) tempActiveOptions.italic = true;
    else tempActiveOptions.italic = false;

    if (currentStyle.has(inlineOptions.underline))
      tempActiveOptions.underline = true;
    else tempActiveOptions.underline = false;

    if (currentStyle.has(inlineOptions.strikethrough))
      tempActiveOptions.strikethrough = true;
    else tempActiveOptions.strikethrough = false;

    if (currentStyle.has(inlineOptions.highlight))
      tempActiveOptions.highlight = true;
    else tempActiveOptions.highlight = false;

    if (currentStyle.has(inlineOptions.code)) tempActiveOptions.code = true;
    else tempActiveOptions.code = false;

    if (blockType === blockOptions.ul) tempActiveOptions.ul = true;
    else tempActiveOptions.ul = false;

    if (blockType === blockOptions.ol) tempActiveOptions.ol = true;
    else tempActiveOptions.ol = false;

    if (blockType === blockOptions.h1) tempActiveOptions.h1 = true;
    else tempActiveOptions.h1 = false;

    if (blockType === blockOptions.h2) tempActiveOptions.h2 = true;
    else tempActiveOptions.h2 = false;
  ...
  const handleEditorOnChange = (currentState) => {
    ...
    if (onChange) {
      if (contentState?.getPlainText()?.trim()) onChange(stringifyContent);
      else onChange("");
    }
    if (getPlaneTextOnChange) {
      getPlaneTextOnChange(contentState?.getPlainText());
    }
  ...
  const generateEditorOptions = () => {
    if (Array.isArray(hiddenOptions) && hiddenOptions?.length > 0) {
  ...
  useEffect(() => {
    ...
    try {
      const defaultState = EditorState.createWithContent(
        convertFromRaw(JSON.parse(defaultState))
      );
  ...
  const styleMap = {
    [inlineOptions.highlight]: {
      backgroundColor: colors?.highlight || "#faed27",
    },
    [inlineOptions.code]: {
      // border: "1px solid #1d1c1d21",
      borderRadius: "3px",
      fontFamily: "monospace",
      color: colors?.code || "#e01e5a",
    },
  };
  ...
  return (
    <div
      ref={containerRef}
      className={`${readOnly ? styles.readOnly : ""} ${styles.container} ${
        error ? (errorClassName ? errorClassName : styles.error) : ""
      } ${className ? className : ""}`}
    >
      {!readOnly && !toolbarToBottom ? (
        <>
          {toolbar}
